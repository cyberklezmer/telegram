...
for (i in 1:nrow(top_channels)) {
  ...
  grouped_data_summary <- grouped_data %>%
    summarize(
      ...
      reactions_with_sentiment = sum(if_else(!is.na(Neutral), count, 0), na.rm = TRUE), # Summing counts with condition
...
#  catalogue_table <- tableGrob(extended_data, rows = NULL, theme = ttheme_default())
  ...
  message_totals <- grouped_data %>%
    ...
    summarize(
      reaction_rows = n(),                                      # Count total rows
      reactions_with_sentiment = sum(if_else(!is.na(positivity), count, 0), na.rm = TRUE), # Summing counts with condition
      # Count non-NA values in neutral
      total_reactions_count = sum(count, na.rm = TRUE),
      total_neutrality = sum(neutrality, na.rm = TRUE),
      total_postitivity = sum(positivity, na.rm = TRUE),
